...
@compute @workgroup_size(256) fn spawn(
    ...
    particle.color = emitter.color_init;
    particle.final_color = emitter.color_final;
    particle.scale = emitter.scale_init;
...
@compute @workgroup_size(256) fn update_state(
    ...
    let t = clamp(next.age / next.death_age, 0.0, 1.0);
    next.scale = mix(current.scale, current.final_scale, t);
    next.final_scale = current.final_scale;
    next.color = mix(current.color, current.final_color, t);
...
@vertex fn vs(
    ...
    let pos = particle.pos + particle.scale * vert_offet;

    output.position = vpMatrix * vec4(pos, 0.0, 1.0);
